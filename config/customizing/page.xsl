...
<xsl:template name="MOBTag">
    ...
    <xsl:when test = "substring-after($data,'vimeo.com') != ''">
      ...
      <iframe frameborder="0" allowfullscreen="1">
        ...
        <xsl:if test="contains($data, 'player.vimeo.com/video')">
          <xsl:attribute name="src">
            <xsl:value-of select="$httpprefix"/>//player.vimeo.com/video/<xsl:value-of select="substring-after($data, 'player.vimeo.com/video/')" />
          </xsl:attribute>
        </xsl:if>
        <xsl:if test="not(contains($data, 'player.vimeo.com/video'))">
          <xsl:attribute name="src">
            <xsl:value-of select="$httpprefix"/>//player.vimeo.com/video/<xsl:value-of select="substring-after($data, 'vimeo.com/')" />
          </xsl:attribute>
        </xsl:if>
        <xsl:comment>Comment to have separate iframe ending tag</xsl:comment>
      </iframe>
